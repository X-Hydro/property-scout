...
def join(geojson_path: str, csv_path: str, out_path: str):
    """
    LEFT join: every GRANIT parcel is kept in the output, whether or not a
    VGSI assessment was found for it. An unmatched parcel gets
    total_market_value/vgsi_location/mblu/acres/land_use_desc all None and
    match_method None, rather than being dropped -- geometry with no known
    dollar value is still a real, usable parcel (comps/target-resolution
    downstream already handle a null assessed_value gracefully; a missing
    polygon is not recoverable the same way a missing value is).

    Previously this was an INNER join (`continue`d past unmatched features
    instead of writing them) -- confirmed as the actual cause of parcels
    disappearing from Postgres despite GRANIT having them: a parcel whose
    VGSI match failed (e.g. an unnumbered subdivision lot skipped by Pass
    2's blank-StreetAddress check) lost its polygon entirely, not just its
    value fields.
    """
    ...
    for feature in parcels["features"]:
        ...
        if assessment is None:
            ...
            props["vgsi_location"] = None
            props["mblu"] = None
            props["acres"] = None
            props["land_use_desc"] = None
        ...
        joined_features.append(feature)  # kept either way -- see docstring
    ...
    print(f"Unmatched GRANIT parcels (kept, with null value fields): {len(unmatched_parcel_pids)}")
    ...
    print(f"Value-match rate: {match_rate:.1%}")
    if match_rate < 0.8:
        print("WARNING: value-match rate below 80% -- spot check a few unmatched PIDs "
              "on both GRANIT and VGSI. All parcels are still written to output (this "
              "is a left join), but a low match rate means a lot of geometry with no "
              "assessed value, which is worth understanding, not just accepting.")

    print(f"Wrote {total_parcels} parcels to {out_path} ({matched} with a VGSI value match, "
          f"{len(unmatched_parcel_pids)} geometry-only).")
